...
localeUses24HourTime = () => new Intl.DateTimeFormat(navigator.language, {
  hour: 'numeric'
...
let drawArbitrary = () => {} // iife for async
;
...
(async () => {
  ...
  const drawMonth = month => ctx.drawImage( // source image
  ...
  const drawWeekday = weekday => ctx.drawImage(weekdays, 0, weekday * 6, 20, 6, 22, 23, 20, 6);
  /**
  ...
  const drawDate = date => {
    if (date > 10) {
      // e.g. 25 -> 2
      const tens = Math.floor(date / 10); // e.g. 25 -> 5

      const ones = date % 10; // draw!
      // tens
  ...
  const updateClock = now => {
    ...
    const minute = now.getMinutes(); // date
